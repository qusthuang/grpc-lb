	...
	go func() {
		...
		for {
			...
			for _, child := range children {
				...
				//{"host":"192.168.2.150","port":2333,"weight":100,"status":1,
				//"version":"3.4.9","metadata":{"weight":"100","version":"3.4.9","status":"true"}}
				nodeData := myregistry.ServiceNodeInfo{}
				err = json.Unmarshal(data, &nodeData)
				if err != nil {
					continue
				}
				addrs = append(addrs, resolver.Address{Addr: nodeData.host + ":" + nodeData.port, Metadata: &nodeData.Metadata})
			}
